...
Private Home Assistant OS add-on for running the official OpenClaw gateway on a Raspberry Pi 5 (`aarch64`) while using LM Studio on another machine on the local network.

## Security model

- Official OpenClaw image only, pinned to version `2026.9.4` and ARM64 image digest.
...
Before first start configure the add-on options. See `openclaw/DOCS.md`.
